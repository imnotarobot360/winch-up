-- Winch Up :: the offers panel disappeared the moment the first helper was accepted
--
-- 20260923002300 made a second helper possible in the database. This is the other half: both the
-- RPC and the status page stopped showing outstanding offers as soon as accepted_responder_id was
-- set, so there was no button to press even once pressing it worked.
--
-- Two gates, one behaviour, and finding only the first would have left the feature still broken
-- while looking fixed.
...
begin
  ...
  result := jsonb_build_object(
    ...
    -- Offers outstanding on a LIVE recovery, whether or not somebody has already been accepted.
    --
    -- This used to stop the moment accepted_responder_id was set, on the reasoning that once
    -- somebody is assigned, a list of people who were passed over is not information the
    -- requester needs. That reasoning belonged to the one-winner model. Under a team they have
    -- not been passed over -- 20260923002300 leaves their offer standing -- and the requester
    -- wanting the tractor as well, ten minutes later once they have seen how buried they are, is
    -- the normal case this phase exists for. Hiding the list takes that choice away.
    --
    -- Still only 'offered' rows, so somebody who withdrew or was declined does not reappear.
    'offers', case when r.status in ('submitted', 'dispatching', 'unmatched', 'accepted', 'on_site')
              then coalesce((
      select jsonb_agg(jsonb_build_object(
               'id',             d.id,
