-- Winch Up :: a recovery request belongs to an account
--
-- Owner's decision: requesters must register. create_request now requires the account id and
-- stores it on the row.
--
-- Two things kept deliberately:
--
--  * The account requirement is checked AFTER the idempotency guard. A retried submit has to
--    return the original request, not fail differently the second time.
--  * public_token is still how the status page is reached. An account identifies who filed the
--    request; it is not the credential for reading it. The requester may be on a different
--    phone, or hand the link to whoever is with them.
--
-- admin_create_request is untouched: a group admin filing on behalf of someone who phoned in has
-- no account to attach, which is why requests.requester_user_id stays nullable.
